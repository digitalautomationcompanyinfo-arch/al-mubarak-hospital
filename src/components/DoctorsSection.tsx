  ...
  return (
    <section className="space-y-8 animate-fade-in">
      ...
      <div className="text-center max-w-3xl mx-auto space-y-3">
        <span className="text-emerald-800 font-extrabold text-xs uppercase tracking-widest bg-emerald-100 px-4 py-1.5 rounded-full border border-emerald-200 shadow-sm inline-flex items-center gap-1.5">
        ...
        {[
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer border ${
              filter === f.id
                ? "bg-red-800 text-white border-red-900 shadow-md shadow-red-900/15"
          ...
          <ScrollReveal key={doc.id} direction="fade-up" delay={idx * 80}>
            <div className="bg-white border border-slate-200/80 rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group flex flex-col justify-between h-full">
                ...
                <div className="relative aspect-[4/3] bg-slate-100 overflow-hidden">
                  ...
                  <div className="absolute top-3 start-3">
                    {doc.isVisiting ? (
                      ...
                      <span className="px-3 py-1 bg-emerald-700 text-white text-[10px] font-bold rounded-full shadow-md">
                ...
                <div className="p-5 space-y-3">
                  ...
                  <div className="pt-2 border-t border-slate-100 space-y-1.5">
                    <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                      <DynamicIcon name="Clock" size={13} className="text-emerald-600" />
                      <span>{lang === "ar" ? "مواعيد التواجد والعيادة:" : "Clinic Availability:"}</span>
                    </p>
                    <div className="flex flex-wrap gap-1">
                      {doc.availableDays.map((day, dIdx) => (
                        <span key={dIdx} className="px-2 py-0.5 bg-emerald-50 text-emerald-800 text-[10px] font-bold rounded-md border border-emerald-100">
                          {day}
                        </span>
                      ))}
                    </div>
              ...
              <div className="p-5 pt-0">
                <button
                  onClick={onBookClick}
                  className="w-full py-2.5 bg-slate-50 hover:bg-emerald-600 hover:text-white border border-slate-200 hover:border-emerald-600 text-slate-800 text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer shadow-sm"
                >
